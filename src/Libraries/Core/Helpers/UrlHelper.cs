...
namespace Core.Helpers
{
    ...
    {
        public static string GetSlugUrl(string title, string newsId, bool remapToAscii = false, int maxlength = 150)
        {
            if (title == null)
            {
                return string.Empty;
            ...
            if (prevdash)
            {
                lastTitle = stringBuilder.ToString().Substring(0, stringBuilder.Length - 1);
            }
            else
            {
                lastTitle = stringBuilder.ToString();
            }
            return $"{lastTitle},{newsId}";
        }
